...
class KernelProvider:
    def __init__(self, precision_policy=None):
        ...
        def write_population_to_global(
            f: wp.array4d(dtype=store_dtype), f_local: vec, x: wp.int32, y: wp.int32
        ):
            for i in range(9):
        ...
        def write_vec_to_global(
            array: wp.array4d(dtype=store_dtype),
            array_local: Any,
            x: wp.int32,
            y: wp.int32,
            dim: wp.int32,
        ...
        @wp.kernel
        def copy_populations(
            origin: wp.array4d(dtype=store_dtype),
            dest: wp.array4d(dtype=store_dtype),
            dim: wp.int32,
        ):
            i, j, k = wp.tid()
            for l in range(dim):
        ...
        @wp.kernel
        def multiply_populations(
            f: wp.array4d(dtype=store_dtype), factor: compute_dtype, dim: wp.int32
        ):
            i, j, k = wp.tid()
            for l in range(dim):
        ...
        def subtract_populations(
            a: wp.array4d(dtype=store_dtype),
            b: wp.array4d(dtype=store_dtype),
            c: wp.array4d(dtype=store_dtype),
            dim: wp.int32,
        ):
            i, j, k = wp.tid()
            for l in range(dim):
                c[l, i, j, 0] = store_dtype(
                    compute_dtype(a[l, i, j, 0]) - compute_dtype(b[l, i, j, 0])
                )
        ...
        def relaxation(
            ...
            for l in range(dim):
                f_destination[l, i, j, 0] = store_dtype(
                    gamma
                    * (
                        compute_dtype(f_after_stream[l, i, j, 0])
                        - compute_dtype(defect_correction[l, i, j, 0])
                    )
        ...
        def interpolate_through_macroscopics(
            fine: wp.array4d(dtype=store_dtype),
            macroscopics_coarse: wp.array4d(dtype=store_dtype),
            ...
            macr_b = read_local_population(
                macroscopics_coarse,
                wp.mod(coarse_i + shift_x + coarse_nodes_x, coarse_nodes_x),
                coarse_j,
            )
            macr_c = read_local_population(
                macroscopics_coarse,
                coarse_i,
            ...
            macr = compute_dtype(0.0625) * (
                compute_dtype(9.0) * macr_a
                + compute_dtype(3.0) * macr_b
                + compute_dtype(3.0) * macr_c
                + compute_dtype(1.0) * macr_d
            )
            ...
            m_fine[7] = (
                (-theta * (one + two * tau_f))
                / ((one + theta) * (tau_s - tau_f))
                * s_s
                * compute_dtype(0)
            )
            m_fine[8] = compute_dtype(0.0) * compute_dtype(0)
        ...
        def interpolate_through_moments_no_boundaries(
            fine: wp.array4d(dtype=store_dtype),
            coarse: wp.array4d(dtype=store_dtype),
            coarse_nodes_x: wp.int32,
            coarse_nodes_y: wp.int32,
        ):
            i, j, k = wp.tid()
            ...
            m_fine = compute_dtype(0.0625) * (
                compute_dtype(9.0) * m_a
                + compute_dtype(3.0) * m_b
                + compute_dtype(3.0) * m_c
                + compute_dtype(1.0) * m_d
            )
        ...
        @wp.kernel
        def check_for_nans(
            f: wp.array4d(dtype=store_dtype), boundary_array: wp.array4d(dtype=wp.int8)
        ):
            i, j, k = wp.tid()
        ...
        @wp.kernel
        def convert_moments_to_populations(
            m: wp.array4d(dtype=store_dtype), f: wp.array4d(dtype=store_dtype)
        ):
            i, j, k = wp.tid()
        ...
        @wp.kernel
        def convert_populations_to_moments(
            f: wp.array4d(dtype=store_dtype), m: wp.array4d(dtype=store_dtype)
        ):
            i, j, k = wp.tid()
        ...
        self.interpolate_through_moments_with_boundaries = (
            interpolate_through_moments_with_boundaries
        )
